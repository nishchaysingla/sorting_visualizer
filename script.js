...
function new_array() {
    // Store current body HTML content
    previousContent = document.body.innerHTML;

    // Replace body content with new array input form
    document.body.innerHTML = `
        <form id="newArrayForm">
            <label for="arr_elements"><h1>Enter all elements of an array:</h1></label><br>
    ...
    document.getElementById("submitBtn").addEventListener("click", function (event) {
        ...
        const arra = document.getElementById("arr").value;
        var array = arra.split(' ');
        document.body.innerHTML = previousContent; // Restore previous body content
        var temp = document.getElementById("elements");
        temp.innerHTML = '';
        for (var i = 0; i < array.length; i++) {
            temp.innerHTML += `<li>${array[i]}</li>`;
        }
        makehistogram(); // Redraw histogram with new array data
    });
}

function goBack() {
    document.body.innerHTML = previousContent; // Restore previous body content
    makehistogram(); // Redraw histogram after going back
}
